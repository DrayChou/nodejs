...
var viewEngine = {
    render: function(req, res, viewName, context){
        var filename = path.join(path.dirname(__dirname), 'views', viewName);
		//console.log('get viewEngine:' + filename);
        try{
...
var staticFileServer = function(req, res, filePath){
    if(!filePath){
        filePath = path.join(path.dirname(__dirname), config.staticFileDir, url.parse(req.url).pathname);
    }
